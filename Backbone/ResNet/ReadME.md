...
## 代码注解
网络模型代码参考 Pytorch 官方实现代码

### 残差模块

残差结构代码如下，其中 downsample 用于调节连接数据的尺寸，前向传播部分在 forward 函数中进行
...
对于 ResNet-50/101/152 使用下面的残差结构，使用 **1 * 1卷积核来升维降维，减少计算参数**

`expansion = 4` 是在 ResNet-50/101/152 中，每个 Block 的最后输出通道数都是输入的 4 倍
...
使用上面两个 Block 进行堆叠，构建网络层 Conv_x 的代码如下
